#!/usr/bin/env node
/**
 * Push to 2000 skills - comprehensive search
 */
...
async function massiveSearch(octokit: Octokit): Promise<string[]> {
  console.log("🔍 Massive search across GitHub...\n");
...
async function addSkillsFromRepo(
  ...
  try {
    ...
    if (error.status === 403) {
      console.log(`   ⏸️  Rate limit - pausing`);
...
async function pushTo2000() {
  console.log(`🎯 Target: 2000 SKILLS!\n`);
  ...
  console.log(`📈 Target: 2000 skills (need ${2000 - startCount} more)\n`);

  const repos = await massiveSearch(octokit);
  ...
  for (const fullName of repos) {
    ...
    if (added > 0) {
      ...
      if (processedCount % 20 === 0) {
        ...
        const percent = ((current / 2000) * 100).toFixed(1);
        console.log(`\n--- ${current}/2000 (${percent}%) | ${processedCount} repos ---\n`);
      }
    }
  }
